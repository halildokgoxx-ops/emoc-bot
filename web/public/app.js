...
function cizSeviye(c){
  ...
  +'<div class="panel" style="margin-top:16px"><div class="panel-top"><div><h3>🎯 Günlük Görev</h3><p>Üyeler hedefi tamamlayınca coin kazanır. Tür: mesaj, ses (1dk=1 puan) veya ikisi.</p></div>'
  +'<label class="tgl"><input type="checkbox" data-k="gorevAktif"'+(f.gorevAktif!==false?' checked':'')+'><span class="ray"></span></label></div>'
  +'<div class="field"><div class="row3"><div><label>Tür</label><select data-k="gorevTur"><option value="mesaj"'+((f.gorevTur||'mesaj')==='mesaj'?' selected':'')+'>💬 Mesaj</option><option value="ses"'+(f.gorevTur==='ses'?' selected':'')+'>🎙️ Ses</option><option value="ikisi"'+(f.gorevTur==='ikisi'?' selected':'')+'>💬+🎙️ İkisi</option></select></div><div><label>Günlük hedef (puan)</label><input type="number" data-k="gorevGunlukMesaj" value="'+(f.gorevGunlukMesaj??20)+'"></div><div><label>Günlük ödül (coin)</label><input type="number" data-k="gorevGunlukOdul" value="'+(f.gorevGunlukOdul??200)+'"></div></div></div>'
  +'<div class="field"><label>Duyuru kanalı (boş = olduğu yerde cevap)</label><select data-k="gorevKanal"><option value="">Kapalı (cevap)</option>'+KANALLAR.filter(k=>k.tip==='yazi').map(k=>'<option value="'+k.id+'"'+(String(f.gorevKanal)===String(k.id)?' selected':'')+'># '+esc(k.ad)+'</option>').join('')+'</select></div></div>'
  +'<div class="panel" style="margin-top:16px"><div class="panel-top"><div><h3>🏆 Haftalık Görev</h3><p>Hafta boyu biriken ilerleme, pazartesi sıfırlanır.</p></div>'
  +'<label class="tgl"><input type="checkbox" data-k="gorevHaftalikAktif"'+(f.gorevHaftalikAktif?' checked':'')+'><span class="ray"></span></label></div>'
  +'<div class="field"><div class="row3"><div><label>Haftalık hedef (puan)</label><input type="number" data-k="gorevHaftalikMesaj" value="'+(f.gorevHaftalikMesaj??100)+'"></div><div><label>Haftalık ödül (coin)</label><input type="number" data-k="gorevHaftalikOdul" value="'+(f.gorevHaftalikOdul??1000)+'"></div><div><label>&nbsp;</label><div class="hint">Bot komutu: <code>!gorev-ayarla gunluk ac</code> (sadece aç/kapa!)</div></div></div></div></div>'
  +saveBar();
}
